...
def send_whatsapp(doc, state):
	...
	for user in user_data_map:	
		...
		# 	"possible_actions":user_data_map[user]["possible_actions"]
		# 	})
	...
	data = json.dumps(user_data_map)
	# res = make_post_request(url = url_host + "/send-workflow", data= user_data_map  )
	resp = req.post(url_host + "/send-workflow", data, headers=headers)
